...
async def run_test():
    try:
        ...
        subprocess.run(
            ["docker", "compose", "up", "-d", "--build", "master", "secondary1"],
            check=True,
            capture_output=True,
        )
        ...
        if all(msg in s2_contents for msg in required_contents):
            log.info(f"s2 has required messages: {s2_contents}")
        else:
            log.error(f"expected {required_contents}, got {s2_contents}")
            return False
